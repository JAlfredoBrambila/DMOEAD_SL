...
import algorithm.reinforcementlearning.QLearningBase;
import algorithm.reinforcementlearning.SARSABase;
import algorithm.reinforcementlearning.SARSALambdaBase;
import algorithm.reinforcementlearning.SARSALambdaBase2;
 ...
 */
public class DMOEAD_SARSALambda implements DynamicMultiobjectiveAlgorithm<Individual>{
    ...
    public DMOEAD_SARSALambda(DynamicProblem problem, int maxIt, int nPOP, int nArchive, double gamma, double mu, double zeta, CrossoverOperator crossover, MutationOperator mutation) {
    ...
    public DMOEAD_SARSALambda(DynamicProblem problem, int maxIt, int nPOP, int nArchive, double gamma, double mu, double zeta, CrossoverOperator crossover, MutationOperator mutation, boolean experimentation, int exp ) {
        this.problem = problem;
        this.nVar = problem.getNumVars();
        this.varMin = problem.getLowerBound();
        this.varMax = problem.getUpperBound();
    ...
    public void execute() {
        ...
        nombreAlgoritmo = "DMOEAD_SARSALambda_";
        
        
        SARSALambdaBase2 sarsa = new SARSALambdaBase2(2,numStrategies,0.04,0.08,0.07);
        int St;
        int St1;
        ...
        for(int it = 0; it<this.maxIt*5; it++) {
            St = 0; // Initial State SL
            action = sarsa.chooseAction(St,0.1); /// ***
            sarsa.updateEligibilityTraces(St, action, 1);
            //double currentQValue = sarsa.getQValue(St, action); /// ***
            for(int i=0; i<this.nPop; i++) {
                
                
                //uso[action]++;
                usage[action] = usage[action]+1;
                y = this.matingEvolution(action, i); /// ***
                ...
                for(double j : sp.get(i).getNeighbors()) {
                    ...
                    if(y.getG() <= pop.get((int)j).getG()) {
                        ...
                        St1 = sarsa.getNextState(St); /// ***
                        nextAction = sarsa.chooseAction(St1,0.1); /// ***
                        double reward = sarsa.getReward(St, action); /// ***
                        
                        //double nextQValue = sarsa.getQValue(St1, nextAction); /// ***
                        
                        sarsa.updateEligibilityTraces(St, action); /// ***
                        sarsa.updateQTable(reward, St, action, St1, nextAction); /// ***
                        
                        //sarsa.decayEligibilityTraces(); /// ***
                        
                        St = St1; /// ***
                        action = nextAction; /// ***
                        //currentQValue = nextQValue; /// ***
                    } else {
                        //St1=1;
                        St = 1;
            ...
            if (hasChangedObjectiveFunction(lookouts)) {
                ...
                */
                ...
                /// ***
                sarsa.initTable(); // inicializar Q Table en ceros 
    ...
    public Individual matingEvolution(int strategySelected, int i) {
        ...
        //System.out.println("Estrategy: " + (strategySelected+1));
        //strategySelected = 4;
    ...
    /*
    public Individual operator5(int i) {
