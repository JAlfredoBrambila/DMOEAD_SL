...
public class DF12 implements DynamicProblem {
    ...
    */
    ...
    public void costFunction(Individual ind) {
        ...
        //double H = 2.25d + 2.0d * Math.cos(0.5d * Math.PI * this.time);
        double k = 10 * Math.sin(Math.PI * this.time);
        ...
        for(int i=2; i<n; i++) {
            ...
            //g+= Math.pow( ind.getPosition()[i] - ((Math.sin(2 * Math.PI * (ind.getPosition()[0] + ind.getPosition()[1])))/1+Math.abs(G)) , 2);
            g += Math.pow((ind.getPosition()[i] - Math.sin(this.time * ind.getPosition()[0])), 2);
        }
        
        g += Math.abs( Math.sin(Math.floor(k * (2 * ind.getPosition()[0])) * (Math.PI/2)) * Math.sin(Math.floor(k * (2 * ind.getPosition()[1])) * (Math.PI/2)) );
        
        f[0] = g * Math.cos(0.5d * Math.PI * ind.getPosition()[0]) * Math.cos(0.5d * Math.PI * ind.getPosition()[1]);
        f[1] = g * Math.cos(0.5d * Math.PI * ind.getPosition()[0]) * Math.sin(0.5d * Math.PI * ind.getPosition()[1]);
        f[2] = g * Math.sin(0.5d * Math.PI * ind.getPosition()[0]);
